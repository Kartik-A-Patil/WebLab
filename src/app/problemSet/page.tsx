            ...
            {loading ? (
              <div className="w-full flex flex-row flex-wrap justify-evenly">
                {Array.from({ length: 4 }, (_, index) => (
                  <Skeleton key={index} className="h-[220px] w-[300px] rounded-lg bg-gray-800 mx-2.5 my-3" />
                ))}
              </div>
            ) : (
              elements.map((comp, index) => (
                <div
                  key={index}
                  className="border h-56 mx-2.5 my-3 rounded-lg border-gray-800 relative hover:border-gray-700 transition-all duration-200"
